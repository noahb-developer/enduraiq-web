...
>
> Last updated: 2026-05-19 (round 10b = Coach page reborn as full-screen Claude-style chat + two mobile form fixes). **Frontend v67.** Round 10a (v66) did the mobile gutter fix. Round 10b: Coach tab is now a fixed full-screen chat pinned under the top nav (AI replies = plain text no bubble, athlete messages = bubble, composer floats above the keyboard via visualViewport), shows latest 300 messages (older stay in DB, just not rendered), the two top boxes (hero + persona badge) deleted, and the Plan Management panel MOVED to the Plan tab under "Share your plan". Plus: Log-manually form labels shortened + Duration got a "min" suffix + number-spinner arrows killed globally (fixes the overlapping/misaligned boxes); workout-feedback RPE row now shrinks to fit (was running off-page). Earlier: round 9 (v65) fixed a TDZ crash that broke ALL line charts, history wrong-workout, classification races. **Strava confirmed they'll respond to the quota increase request soon (awaiting).**
>
